...
def _lease_liability_amount_evidence(report: FullReport) -> list[str]:
    evidence: list[str] = []
        ...
        for block in section.blocks:
            ...
            for row_idx, row in enumerate(table.rows[1:], start=1):
                if not row or not _is_explicit_lease_liability_amount_label(row[0]):
                    continue
                amount_col = _first_amount_col(row)
                if amount_col is None:
                    continue
                evidence.append(
                    f"{row[0]} @ {section.section_id}/table:{table.index}/row:{row_idx}/col:{amount_col}"
                )
    return evidence
...
def _lease_maturity_disclosure_state(report: FullReport) -> _DisclosureSearchState:
    ...
    for item in build_note_inventory(report).tables:
        ...
        if _is_lease_maturity_like_table(item):
            backlog.append(
...
def _first_amount_col(row: list[str]) -> int | None:
...
def _is_lease_maturity_like_table(item: NoteTableInventoryItem) -> bool:
    values = (
        item.title,
        item.heading,
        *item.headers,
    ...
    if "리스부채" not in joined:
        return False
    if any(token in title_heading for token in ("만기분석", "계약상만기")):
        return True
    if "유동성위험" in title_heading:
        return True
    return _count_maturity_bucket_labels((*item.headers, *item.row_labels)) >= 2
