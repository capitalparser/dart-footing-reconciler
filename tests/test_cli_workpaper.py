...
def test_cli_workpaper_html_runs_validation_qa(tmp_path):
    source = tmp_path / "report.html"
    source.write_text(
        """
        <p>12. 유형자산</p>
        <table>
          <tr><th>구분</th><th>건물</th><th>기계장치</th><th>합계</th></tr>
          <tr><td>기초</td><td>40</td><td>60</td><td>100</td></tr>
          <tr><td>취득</td><td>10</td><td>20</td><td>30</td></tr>
          <tr><td>합계</td><td>50</td><td>80</td><td>130</td></tr>
        </table>
        """,
        encoding="utf-8",
    )
    ...
    result = CliRunner().invoke(app, ["workpaper-html", str(source), str(output), "--company", "Sample Co"])

    assert result.exit_code == 0
    assert "QA " in result.output
    assert "검증 커버리지 QA" in output.read_text(encoding="utf-8")


def test_cli_qa_report_emits_validation_coverage_summary(tmp_path):
    source = tmp_path / "report.html"
    source.write_text(
        """
        <p>현금흐름표</p>
        <table><tr><th>구분</th><th>당기</th></tr><tr><td>유형자산의 취득</td><td>(500)</td></tr></table>
        <p>12. 유형자산</p>
        <table><tr><th>구분</th><th>당기</th></tr><tr><td>취득</td><td>500</td></tr></table>
        """,
        encoding="utf-8",
    )

    result = CliRunner().invoke(app, ["qa-report", str(source), "--company", "Sample Co"])

    assert result.exit_code == 0
    assert "QA " in result.output
    assert "cashflow_coverage" in result.output
