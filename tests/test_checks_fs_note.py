...
def test_fs_note_lease_not_tested_carries_no_applicable_check_reason():
    # Lease pairing was attempted but no level/target resolved — this is 미검증
    # (could not conclude), not 해당없음 (no lease at all). C1 must tag it.
    bs = _lease_statement(
        [
            ["구분", "당기"],
            ["유동 리스부채", "100"],
    ...
    not_tested = [result for result in results if result.status == "not_tested"]
    assert not_tested
    assert all(result.not_tested_reason == "no_applicable_check" for result in not_tested)


def test_fs_note_investment_property_absent_reports_not_applicable():
    # A balance sheet with PPE but no investment property anywhere (no statement line,
    # no note) → the tool should positively conclude 해당없음(정상), not stay silent.
    bs = _section(
        "statement:bs",
        "재무상태표",
        "statement",
    ...
    report = FullReport("s.html", "Co", [bs], [])

    results = check_fs_note_matches(report)

    ip = [result for result in results if result.account_key == "investment_property"]
    assert len(ip) == 1
    assert ip[0].status == "not_tested"
    assert ip[0].not_tested_reason == "not_applicable"


def test_fs_note_investment_property_present_does_not_report_not_applicable():
    # Guard against a false 해당없음: when the account IS present, no N/A row is emitted.
    bs = _section(
        "statement:bs",
        "재무상태표",
        "statement",
    ...
    na = [
        ...
        if result.account_key == "investment_property"
        and result.not_tested_reason == "not_applicable"
    ]
    assert na == []


def test_fs_note_bonds_absent_reports_not_applicable():
    # No 사채 amount anywhere (only PPE) → 사채 해당없음. Bonds detection cannot rely on the
    # account classifier (folds 사채 into 차입금); it scans parsed tables for a 사채 amount row.
    bs = _section(
        "statement:bs",
        "재무상태표",
        "statement",
    ...
    b = [result for result in results if result.account_key == "bonds"]
    assert len(b) == 1
    assert b[0].status == "not_tested"
    assert b[0].not_tested_reason == "not_applicable"


def test_fs_note_bonds_amount_row_anywhere_blocks_not_applicable():
    # A 사채 row with an amount anywhere (even folded in a 차입금 note) → bonds present →
    # never a false 해당없음, no matter how the account classifier keyed it.
    bs = _section(
        "statement:bs",
        "재무상태표",
        "statement",
    ...
    report = FullReport("s.html", "Co", [bs], [note])
    ...
    na = [
        result
        for result in results
        if result.account_key == "bonds" and result.not_tested_reason == "not_applicable"
    ]
    assert na == []
