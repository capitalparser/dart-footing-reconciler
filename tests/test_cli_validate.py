...
def test_cli_validate_excel_writes_workbook(tmp_path) -> None:
    source = tmp_path / "report.html"
    source.write_text(
        """
        <p>11. 유형자산</p>
    ...
    manifest = tmp_path / "manifest.json"
    manifest.write_text(
        json.dumps({"samples": [{"name": "sample", "source": source.name}]}),
        encoding="utf-8",
    )
    output = tmp_path / "review.xlsx"

    result = CliRunner().invoke(app, ["validate-excel", str(manifest), str(output)])
