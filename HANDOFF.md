...
**2026-06-11 3차 슬라이스(사용자 UI 피드백 6건 반영):**
1. **cockpit 제거 + 검증 로직 첫 단** — 검증 목표 Cockpit/tickmark 범례/작업·리뷰 뷰탭/KPI 스트립 제거. 첫 단에 `logic-brief`(6개 검증 축 설명 + 상태 배지 범례 + 전체 건수 요약). 리뷰 큐·커버리지는 본문 뒤 일반 섹션으로 이동.
2. **주석 패널 순서** — 원문(문단·표 인터리브, 표 카드 기본 펼침 + 표마다 검증 배지/셀 하이라이트) 먼저, 그 하단에 본문 대사·합계·전기대사 패널.
3. **표 밀도** — 빈 행 검증 셀은 한 줄 배지로 축소(장황 문구 → title 툴팁), 첫 열 라벨 3줄 클램프, 검증 칼럼 폭 13/17/9%로 축소.
4. **표 구조 해석 보완(checks_totals)** — (i) `_amount_cell` 엄격화(비율·내용연수·기간 셀 제외), (ii) 합산 구조 없는 표(내용연수·할인율·가정·2행 단편)는 not_tested로 강등, (iii) 다단 헤더 단일 '합계' 열을 leaf 라벨 배타성 가드 하에 행 합계로 승격, (iv) rows[0] 합계 라벨이 있어도 다단 헤더면 가드 경로 강제 — 측정 요약 표(토지/건물 그룹 혼합)의 거짓 합계 차이 제거. 다중 합계 그룹(구간 합계·중간 소계)은 의도적으로 보류(세그먼트 일반화 시도는 거짓 차이 +86으로 즉시 롤백).
5. **scope-group 내부 요소 숨김 버그** — 주석 표 카드(data-report-scope="unknown")가 스코프 전환 JS에 숨던 문제: 그룹 내부 요소는 그룹 단위 표시가 관장하도록 수정.
6. **10개사 corpus 재실행** — parse_uncertain 664→569, unexplained_total_check 342→260, unexplained_gap 709→627, not_tested 2,789→3,074(정직한 재분류), primary 110/141·false-match review 15 유지. matched 감소(4,565→4,101)는 다단 헤더 우연 합산 매칭의 보류 전환분 — false-match 최소화 우선 독트린에 따른 의도적 트레이드오프.

**남은 항목:** (a) fs_note 잔여 갭은 의미적 브리지 필요 사례(감가상각비 PPE분 vs 전체분, EPS 분자/주당 혼동, 단위 혼합 컬럼 추출) — expense-allocation 브리지 확장으로 다룰 것. (b) `_note_area_scope`의 문장 내 '재무제표 주석' 언급 오탐 가능성(기존 동작 계승). (c) 결손금처리계산서 실표본 미확보. (d) 다중 합계 그룹 표(연령분석·기간별 합계)의 footing은 span 그룹 해석이 필요한 다음 단계 후보 — 현재는 정직하게 parse_uncertain 유지. (e) 자체 검증 한계 advisory 블록은 테스트 계약 유지를 위해 존치 — logic-brief와 중복이므로 다음 슬라이스에서 통합 검토.
