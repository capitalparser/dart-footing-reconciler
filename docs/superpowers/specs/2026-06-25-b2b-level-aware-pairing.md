...
**Residual:** the current-year-table heuristic ("lowest index = 당기") is document-order-based; a wrong pick degrades to a gap (safe), never a false match. The `자산` isolation token is broad (corpus-harmless). The `_lease_note_hits_by_level` 3rd return value is now always False (compatibility; the ambiguous-abstain path is dead).

**Verification:** 2-leg PLAN review (Opus+Codex, converged) + corpus hard gate + 879 tests. **Formal CODE review run 2026-06-25 (Opus `code-reviewer`; Codex leg interim/best-effort): found 1 BLOCKER + 2 MAJOR false-MATCH paths the corpus shape avoided — all fixed and regression-pinned. See `docs/adr/0013-b2b-code-review-findings.md`.** Corpus gate unchanged after the fixes (baselines identical → out-of-corpus guards): +22 matched, 0 destroyed, 0 false, 0 new gaps.
