...
2. **Thread `consolidation_basis` once via `VerificationContext`.** Compute it per slice in the harness runner (`report_slice.statements|notes[0].scope`, normalized to `consolidated`/`separate`/`unknown`) and have each check copy it onto its `CheckResult`. This covers *all* families cheaply and correctly (the slice is homogeneous).
...
3. **Populate the other three where they are already known; otherwise honest `"unknown"`** (no fabrication, per ADR-0017):
   ...
   - `balance_level`: fs_note lease/borrowings level-aware pairing — emit the level used in the by-level grouping (`current`/`noncurrent`/`total`); `unknown` for the flat (non-level) model.
